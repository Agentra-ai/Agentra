...
const AskMe = (props: Props) => {
  ...
  return (
    <div className="flex h-[calc(100vh-55px)] w-full overflow-hidden bg-[#f3f5f7] p-3 pr-0 text-[14px]">
      <div
        className={`transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-[240px]" : "w-[55px]"} overflow-hidden rounded-l-2xl bg-white shadow-lg`}
      >
        <div className="p-2">
          <h2 className="text-md mb-4 font-semibold text-gray-800">AI</h2>
      ...
      {/* Main Chat Area */}
      <div className="relative flex h-full flex-1 flex-col overflow-hidden shadow-md shadow-gray-200">
        {/* Header - Sticky with toggle button */}
        <div className="sticky top-0 z-10 flex h-[60px] items-center justify-between bg-[#fafcff] px-4 shadow-sm">
          <button
        ...
        {/* Messages - Scrollable */}
        <div className="flex-1 overflow-y-auto bg-[#f3f5f7] px-4">
          <div
            className={`mx-auto max-w-3xl ${isSidebarOpen ? "w-[calc(100%-220px)]" : "w-full"} rounded-l-2xl transition-all duration-300 ease-in-out`}
          >
            <div className="flex flex-col gap-6 py-6">
              {Array.from({ length: 10 }).map((_, index) => (
                <div
                  key={index}
                  className={`flex w-full items-start ${index % 2 !== 0 ? "justify-end" : ""}`}
                >
                  {index % 2 === 0 ? (
                    ...
                    <div className="flex w-full max-w-[85%] justify-end gap-2">
                      <div className="rounded-2xl bg-[#e1eaff] p-3 text-black shadow-sm">
                        ...
                        <div>Hi, I&#39;m ready for the interview...</div>
                      </div>
                      <div className="flex-shrink-0">
        ...
        <div className="sticky bottom-0 w-full bg-[#f3f5f7] pt-2">
          <div
            className={`mx-auto max-w-3xl ${isSidebarOpen ? "w-[calc(100%-220px)]" : "w-full"} pb-2 transition-all duration-300 ease-in-out`}
          >
            <div className="relative flex items-center rounded-xl bg-white p-2 shadow-md">
              <TextArea
