...
} from "../chat/chat-action";
import { and, count, eq, gte, sql } from "drizzle-orm";
import { conversations, messages } from "@/drizzle/schema"; // Adjust import path as needed
...
export const getPastDaysData = async (appId: string, days: number = 7) => {
  ...
  const messageData = await db
    ...
    .where(
      and(eq(conversations.appId, appId), gte(messages.createdAt, startDate)),
    )
  ...
  const completeDataset = dateRange.map((date) => {
    ...
    return {
      ...
      tokenSpeed: messageRow.avgTimestamp
        ? Number(messageRow.totalTokens) / Number(messageRow.avgTimestamp)
        : 0,
    };
  });
  ...
  return {
    conversations: {
      ...
      totalValue: completeDataset.reduce(
        (acc, row) => acc + row.conversationCount,
        0,
      ),
    },
    messages: {
    ...
    completionTokens: {
      ...
      totalValue: completeDataset.reduce(
        (acc, row) => acc + row.completionTokens,
        0,
      ),
    },
    promptTokens: {
    ...
    tokenSpeed: {
      ...
      totalValue:
        completeDataset.reduce((sum, row) => sum + row.tokenSpeed, 0) /
