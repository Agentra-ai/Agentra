...
export async function Header(): Promise<JSX.Element> {
  ...
  // const users = session?.user?.email ? getUserByEmail({ email: session?.user?.email }) : null
    ...
    <header className="sticky top-0 z-40 flex h-20 w-full bg-transparent">
      <div className="container flex items-center justify-between p-4">
        ...
        >
          ...
          />
          <span className="hidden font-anek text-4xl tracking-tight md:flex">
            {siteConfig.name}:
          </span>
        </Link>
          ...
          <nav className="space-x-1">
            {session?.user ? (
              <div className="flex items-center gap-2 ">
                <button
                  onClick={redirect("/apps/explore")}
                  className="flex items-center gap-1"
                >
                  {" "}
                  Dashboard <IoIosArrowDropright />
                </button>
                <DropdownMenu>
                  <DropdownMenuTrigger
                    asChild
